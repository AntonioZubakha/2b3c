...
const redisUrl = resolveSecret('MARKET_PRICE_CACHE_REDIS_URL')
...
async function refreshFromMongo(): Promise<boolean> {
  ...
  if (mongoose.connection.readyState !== 1) {
    logger.warn('[MarketPriceCache] Skipping Mongo fallback because mongoose connection is not ready');
    return false;
  }
