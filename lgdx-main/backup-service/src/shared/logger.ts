/**
 * Централизованная система логирования для Backup Service
 * С маскированием чувствительных данных
 */
...
const redactKeys = new Set([
  ...
  'password', 'secret', 'jwt', 'bearer', 'mongodb_uri', 'uri'
]);
...
function redact(input: any): any {
  try {
    ...
    if (typeof input === 'string') {
      // Маскируем MongoDB URI и другие чувствительные строки
...
function log(level: LogLevel, message: string, meta?: unknown): void {
  ...
  const line = `[${ts}] [backup-service] [${level.toUpperCase()}] ${message}`;
  switch (level) {
    case 'debug':
